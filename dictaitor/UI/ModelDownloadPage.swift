...
struct ModelDownloadPage: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("DictAItor needs to download ML models for transcription.")
                .font(.body)
            ...
            VStack(spacing: 12) {
                ...
                ModelDownloadCard(
                    category: .tts,
                    state: downloadService.ttsState,
                    onRetry: { Task { try? await downloadService.downloadTTS() } },
                    onSkip: { downloadService.skip(.tts) }
                )
    ...
    private func startDownloadsIfNeeded() {
        ...
        Task {
            ...
            if downloadService.ttsState == .idle {
                try? await downloadService.downloadTTS()
            }
