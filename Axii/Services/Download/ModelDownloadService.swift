...
final class ModelDownloadService {
    ...
    init() {
        // All models stored in ~/Library/Application Support/Axii/Models/
        let appSupport = FileManager.default.urls(
            for: .applicationSupportDirectory,
            in: .userDomainMask
        ).first!

        let baseDir = appSupport.appendingPathComponent("Axii", isDirectory: true)
    ...
    func cacheDirectory(for category: ModelCategory) -> URL {
        modelsDirectory.appendingPathComponent(category.folderName, isDirectory: true)
    }
...
extension ModelDownloadService {
    ...
    var isDownloading: Bool {
        asrState.isDownloading || diarizationState.isDownloading
    }
